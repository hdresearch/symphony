defmodule SymphonyElixir.Orchestrator do
  ...
  alias SymphonyElixir.{AgentRunner, Config, StatusDashboard, Tracker, VersAgentRunner, Workspace}
  ...
  defp spawn_issue_with_vers(%State{} = state, issue, attempt, recipient) do
    ...
    case Task.Supervisor.start_child(SymphonyElixir.TaskSupervisor, fn ->
      ...
      {:ok, pid} ->
        ...
        Logger.info("Dispatching issue to agent: #{issue_context(issue)} pid=#{inspect(pid)} attempt=#{inspect(attempt)} runner=vers")
      ...
      {:error, reason} ->
        Logger.error("Unable to spawn Vers agent for #{issue_context(issue)}: #{inspect(reason)}")
        next_attempt = if is_integer(attempt), do: attempt + 1, else: nil

        schedule_issue_retry(state, issue.id, next_attempt, %{
          identifier: issue.identifier,
          error: "vers spawn failed: #{inspect(reason)}"
        })
    end
  end
